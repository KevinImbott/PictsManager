  ...
  void _onItemTapped(int index) async {
    ...
    if (_selectedIndex == 0) {
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => ScreenChoosePicture(cameras: cameras)));
    }
    if (_selectedIndex == 1) {
      savePicture();
    }
  ...
  void toggleCam() {
    ...
    setState(() {
      if (camera) {
        ...
        _controller =
            CameraController(widget.cameras[2], ResolutionPreset.high);
        _initializeControllerFuture = _controller.initialize();
      }
    });
  }
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const <BottomNavigationBarItem>[
          BottomNavigationBarItem(
              icon: Icon(Icons.arrow_back_ios_new, size: 40), label: 'Back'),
          BottomNavigationBarItem(
